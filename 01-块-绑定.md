...
在这里例子中，for-in循环表现出和for循环同样的行为。每次迭代时，创建一个新的key绑定，所以每个函数都有自己的key变量副本。这样的结果是每个函数都输出不同的值。如果使用var声明key，所有的函数将会输出“c”。
...
let和const不同于var的另外一个方面是他们在全局作用域的行为。当在全局作用域使用var时，这就创建了一个新的全局变量，它是全局对象（浏览器中的`window`对象）的一个属性。那就意味着，你可能意外地覆盖一个已经存在的全局var变量，比如：
...
`let`和`const`的块级绑定给JavaScript引入了词法作用域的概念。这种变量声明不会发生变量提升，变量只会存在声明他们所在的作用域。这种特性提供的行为更像其他语言，减少造成意外错误的可能性，因为只需要在需要的地方声明变量。另一个负效应是，在变量声明之前，你不能访问它们，即使是使用安全的操作，比如`typeof`。试图在声明变量之前访问块级绑定，会导致错误，因为变量绑定存在“暂时盲区”（TDZ）。
